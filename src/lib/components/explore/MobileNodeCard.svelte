...
<Portal on:mounted={() => closeButton.focus({ preventScroll: true })}>
  <section class="explore-mobile-card" role="dialog" aria-modal="true" aria-label={label} tabindex="-1" on:keydown={trapTab} use:lockPageScroll>
...
<style>
  :global(body:has(.explore-mobile-card) .mobile-bottom-nav){visibility:hidden}
  .explore-mobile-card{--explore-surface:#fff;--explore-ink:#2d3650;--explore-canvas:#f6f7fb;position:fixed;inset:0;z-index:90;width:100%;height:100dvh;display:flex;flex-direction:column;overflow:hidden;background:var(--explore-surface);color:var(--explore-ink);box-sizing:border-box}
  ...
  :global(.dark) .explore-mobile-card{--explore-surface:#242631;--explore-ink:#d9dfec;--explore-canvas:#1d202a}
</style>
