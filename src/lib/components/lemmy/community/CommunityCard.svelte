...
<aside class="break-words flex flex-shrink flex-col gap-4 !text-sm border-slate-200 dark:border-zinc-800 max-w-[640px] mx-auto w-full pt-0 text-slate-600 dark:text-zinc-400">
  <div class="bg-white dark:bg-zinc-900 rounded-xl p-4">
    <div class="flex items-center gap-2">
      <Avatar
        ...
        class_="w-8 h-8"
      />
      <span class="text-base font-normal text-slate-900 dark:text-zinc-200">
    ...
    <div class="flex flex-col gap-4 mt-4">
      <Expandable bind:open={$userSettings.expand.about} class="!pt-0">
        <span slot="title" class="py-1 px-1 flex gap-1 items-center text-sm text-slate-900 dark:text-zinc-200">
          <Icon src={InformationCircle} size="15" mini />
          {$t('cards.site.about')}
        </span>
        <div class="text-sm font-normal text-slate-700 dark:text-zinc-200">
          <Markdown 
            source={community_view.community.description} 
            allowLinks={true}
          />
        </div>
      </Expandable>
        ...
        <div class="flex flex-row items-center gap-1">
          {#if $profile?.user && amMod($profile.user, community_view.community)}
            <Button
              ...
              size="square-md"
            >
              <Icon src={Cog6Tooth} size="15" mini slot="prefix" />
            </Button>
          {/if}
          ...
          <Menu placement="top-end">
            ...
            >
              <Icon src={NoSymbol} size="15" mini slot="prefix" />
            ...
            {#if $profile?.user}
              ...
              >
                <Icon src={BuildingOffice2} size="15" mini slot="prefix" />
            ...
            {#if $profile?.user && isAdmin($profile.user)}
              ...
              >
                <Icon src={Fire} size="15" mini slot="prefix" />
                {$t('admin.purge')}
              </MenuButton>
            {/if}
          </Menu>
