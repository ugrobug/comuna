...
const DEFAULT_BLOCKS: Record<EditableStaticPageSlug, Array<Record<string, any>>> = {
  about: [
    ...
    paragraph(
      '1. Вы создаёте сообщество на сайте.<br>2. В настройках сообщества привязываете существующий публичный Telegram-канал и подтверждаете его через бота.<br>3. Посты канала появляются в сообществе, а поисковые системы индексируют страницы.<br>4. Читатели приходят на сайт и подписываются на ваш канал.'
    ),
  ],
  advertisement: [
    paragraph(
  ...
  authors: [
    ...
    paragraph(
      '1. <a href="/comuns?create=1">Создайте сообщество на сайте</a>.<br>2. В настройках сообщества → Telegram укажите существующий публичный канал с @username и сохраните.<br>3. Получите код подтверждения там же и отправьте его боту <a href="https://t.me/comuna_tg_bot" target="_blank" rel="nofollow noopener">@comuna_tg_bot</a>.<br>4. Добавьте бота в администраторы канала и выберите в боте режим публикации.<br>5. Проверьте привязку в настройках сообщества. Если канал ещё не выбран, выберите его и сохраните настройки.<br>6. Публикуйте посты как обычно — они появятся в сообществе. Сообщества создаются только на сайте.'
    ),
  ],
  rules: [
    paragraph(
