<script lang="ts">
  ...
  async function uploadImage(event: Event) {
    ...
    const id = editing!
    await action(() => api.uploadImage(id, file), 'Изображение уменьшено и сохранено в WebP. Оригинал не хранится.')
  }
...
<style>
  .image-preview{display:block;width:100%;max-width:400px;height:auto;aspect-ratio:16/9;object-fit:cover;border-radius:12px}
