<script lang="ts">
  ...
  let applicationsCatalogOpen = false
  ...
  const featureApplications: ComunApplication[] = [
    {
      id: 'feature:glossary',
      name: 'Глоссарий',
        ...
        {:else if settingsTab === 'applications'}
          <div class="flex flex-col gap-5">
            <div class="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
              <div class="max-w-3xl">
                <div class="text-lg font-semibold text-slate-950 dark:text-zinc-50">Приложения</div>
                <p class="mt-1 text-sm leading-relaxed text-slate-600 dark:text-zinc-400">
                  «Приложения» — это мини-программы, расширяющие возможности вашего сообщества. Они позволят создавать новые форматы отображения данных в вашем сообществе.
                </p>
              </div>
              <Button on:click={openApplicationsCatalog}>Добавить приложение</Button>
            </div>
            ...
            <div class="flex flex-col gap-2">
              ...
              {#if installedCommunityApplications.length}
                <div class="divide-y divide-slate-200 overflow-hidden rounded-lg border border-slate-200 dark:divide-zinc-800 dark:border-zinc-800">
                  {#each installedCommunityApplications as application (application.id)}
              ...
              {:else}
                <div class="rounded-lg border border-dashed border-slate-300 px-4 py-6 text-sm text-slate-500 dark:border-zinc-700 dark:text-zinc-400">
                  В сообществе пока нет установленных приложений.
                </div>
              {/if}
            </div>
          </div>
...
<Modal bind:open={applicationsCatalogOpen} dismissable={true} dismissOnBackdrop={true}>
  <div class="flex w-full max-w-2xl flex-col gap-4">
    ...
    <div class="max-h-[65vh] divide-y divide-slate-200 overflow-y-auto rounded-lg border border-slate-200 dark:divide-zinc-800 dark:border-zinc-800">
      {#each featureApplications as application (application.id)}
        <div class="flex flex-col gap-3 px-4 py-4 sm:flex-row sm:items-center sm:justify-between">
          ...
          <Button
            size="sm"
            disabled={isApplicationInstalled(application)}
            on:click={() => installApplication(application)}
          >
            {isApplicationInstalled(application) ? 'Установлено' : 'Установить'}
          </Button>
        </div>
      {/each}
    </div>
    ...
    <div class="flex justify-end">
      <Button color="ghost" on:click={closeApplicationsCatalog}>Закрыть</Button>
    </div>
  </div>
</Modal>
